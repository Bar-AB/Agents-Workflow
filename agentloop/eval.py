...
FIXTURES: list[EvalFixture] = [
    ...
    EvalFixture(
        ...
        VerdictKind.APPROVE, _v("approve", 0.95, "pass", "Meets all criteria.")),
    EvalFixture(
        "g-fib", "good", "fibonacci",
        "fib(n) returning the nth Fibonacci number, iterative.",
        "Correct for n>=0, O(n) time, handles n=0/1, tested.",
        "def fib(n):\n a,b=0,1\n for _ in range(n): a,b=b,a+b\n return a",
        VerdictKind.APPROVE, _v("approve", 0.90, "pass", "Correct and tested.")),
    EvalFixture(
        "g-email", "good", "email validator",
        ...
        VerdictKind.APPROVE, _v("approve", 0.82, "pass", "Reasonable and tested.")),
    EvalFixture(
        "g-json", "good", "safe json load",
        ...
        VerdictKind.APPROVE, _v("approve", 0.78, "pass", "Both paths covered.")),
    EvalFixture(
        "g-bsearch", "good", "binary search",
        "bsearch(a, x) -> index or -1, a sorted.",
        "Correct on hit/miss/empty, O(log n), tested.",
        "def bsearch(a,x):\n lo,hi=0,len(a)-1\n while lo<=hi:\n  m=(lo+hi)//2\n"
        "  if a[m]==x: return m\n  if a[m]<x: lo=m+1\n  else: hi=m-1\n return -1",
        VerdictKind.APPROVE, _v("approve", 0.88, "pass", "Standard, correct.")),
    EvalFixture(
        "g-dedup", "good", "stable dedup",
    ...
    EvalFixture(
        ...
        VerdictKind.REVISE, _v("revise", 0.50, "fail",
                               "Empty input divides by zero; add a guard.")),
    EvalFixture(
        "sw-mutates", "subtly_wrong", "sorted copy",
        ...
        VerdictKind.REVISE, _v("approve", 0.80, "pass",
                               "Looks correct, returns sorted list.")),
    EvalFixture(
        "sw-overflow", "subtly_wrong", "percent",
        "pct(a, b) returns a/b as a percentage.",
        "Handles b==0 gracefully, correct otherwise, tested.",
        "def pct(a,b): return a/b*100   # b==0 raises",
        VerdictKind.REVISE, _v("revise", 0.45, "fail",
                               "No guard for b==0.")),
    EvalFixture(
        "sw-none", "subtly_wrong", "title case",
        "titlecase(s) title-cases a string.",
        "Handles None by returning '', tested.",
        "def titlecase(s): return s.title()   # AttributeError on None",
        VerdictKind.REVISE, _v("revise", 0.58, "fail",
                               "None not handled per criteria.")),
    EvalFixture(
        "sw-rounding", "subtly_wrong", "round half up",
        ...
        VerdictKind.REVISE, _v("approve", 0.74, "pass",
                               "round() to 2 dp looks fine.")),
    EvalFixture(
        "sw-notest", "subtly_wrong", "untested parser",
        "parse_port(s) -> int in 1..65535 or raises ValueError.",
        "Range-checked and TESTED per criteria.",
        "def parse_port(s):\n p=int(s)\n return p   # no range check, no tests",
        VerdictKind.REVISE, _v("revise", 0.52, "na",
                               "No range check and no tests, as required.")),

    # -- ambiguous / unsalvageable: gold escalate ---------------------------
    EvalFixture(
        "am-locale", "ambiguous", "locale-sensitive slug",
        ...
        VerdictKind.ESCALATE, _v("escalate", 0.25, "na",
                                 "Criteria don't define the transliteration.")),
    EvalFixture(
        "am-format", "ambiguous", "date format",
        ...
        VerdictKind.ESCALATE, _v("escalate", 0.30, "na",
                                 "'Standard format' is underspecified.")),
    EvalFixture(
        "am-approach", "ambiguous", "wrong problem",
        "Build an LRU cache.",
        "Evicts least-recently-used at capacity.",
        "class Cache(dict): pass   # solves a different problem entirely",
        VerdictKind.ESCALATE, _v("escalate", 0.20, "fail",
                                 "Unsalvageable: no eviction, wrong approach.")),
    EvalFixture(
        "am-criteria", "ambiguous", "missing criteria",
        "Make it 'fast'.",
        ...
        VerdictKind.ESCALATE, _v("revise", 0.48, "na",
                                 "Asks for a benchmark; treats it as fixable.")),
    EvalFixture(
        "am-unsalvage", "ambiguous", "contradictory spec",
        ...
        VerdictKind.ESCALATE, _v("escalate", 0.15, "na",
...
def run_eval(result_store: Store, runner, registry: Registry,
             fixtures: list[EvalFixture] | None = None) -> dict:
    ...
    try:
        for fx in fixtures:
            task = Task(id=None, title=fx.title, goal=fx.goal,
                        acceptance_criteria=fx.criteria)
            scratch.add_task(task)
            verdict, _ = run_validator(scratch, runner, registry, task,
                                       fx.worker_output)
            pred = verdict.kind
            ...
            detail.append({
                ...
                "gold": fx.gold.value, "predicted": pred.value,
    ...
    for i in range(len(CALIBRATION_EDGES) - 1):
        ...
        calibration.append({
            "bucket": _bucket_label(i), "n": b["n"], "correct": b["correct"],
            "accuracy": round(acc, 4) if acc is not None else None,
        })

    summary = {"agreement": round(agreement, 4), "n": n,
               "confusion": confusion, "calibration": calibration}
    result_store.add_eval_run(
        runner=type(runner).__name__, n_fixtures=n, agreement=agreement,
